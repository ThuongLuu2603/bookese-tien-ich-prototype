...
function priceBreakdown(q,qty=1,atCheckout=false){
 ...
 const voucher=atCheckout?voucherResult().discount:0,used=atCheckout&&customer.tier>=0?Math.min(customer.coins,100000,Math.floor((total-voucher)*.3)):0,payable=total-voucher-used;
 const coins=customer.tier<0?0:Math.floor(payable*(customer.tier+1)/100);
 return `<div class="pp-row"><span>Giá gốc</span><del>${money(q.base*qty)}</del></div><div class="pp-applied">${q.rows.map(r=>`<div>${q.rows.length>1?`<small>Đêm ${esc(r.date)}</small>`:''}${r.best.ps.length?r.best.ps.map(p=>`<div class="pp-row"><span>${esc(p.name)}</span><span>−${p.rate}%</span></div>`).join(''):'<div class="pp-row"><span>Không có ưu đãi áp dụng</span></div>'}</div>`).join('')}<small>${atCheckout?'Ưu đãi phòng tính nối tiếp':'Ưu đãi tính nối tiếp · Giá chưa trừ mã hoặc xu'}</small></div><div class="pp-row pp-section"><span>Giá phòng</span><span>${money(net)}</span></div><div class="pp-sub">${nightsHtml}</div><div class="pp-row pp-section"><span>Thuế & Phí</span><span>${money(service+vat)}</span></div><div class="pp-sub"><div class="pp-row"><span>Phí dịch vụ 5%</span><span>${money(service)}</span></div><div class="pp-row"><span>Thuế VAT 10%</span><span>${money(vat)}</span></div></div><div class="pp-total"><div class="pp-row"><span>Tổng tiền phòng</span><span>${money(total)}</span></div><em>(Bao gồm Thuế & phí cho ${nights()} đêm, ${qty} phòng)</em></div>${atCheckout?`<div class="pp-checkout-adjustments"><div class="pp-row"><span>Mã Bookese${voucher?' · '+esc(customer.code):''}</span><span>−${money(voucher)}</span></div><div class="pp-row"><span>Xu sử dụng</span><span>−${money(used)}</span></div><div class="pp-row pp-payable"><b>Tổng thanh toán</b><b>${money(payable)}</b></div></div>`:''}<div class="pp-row pp-coins"><div>Xu nhận sau lưu trú<br><em>${customer.tier<0?'Đăng nhập để nhận xu':'Hạng “'+['Thành viên','Thân thiết','Trung thành'][customer.tier]+'”'}</em></div><span>${customer.tier<0?'—':'+'+coins.toLocaleString('vi-VN')+' xu'}</span></div>`;
}
