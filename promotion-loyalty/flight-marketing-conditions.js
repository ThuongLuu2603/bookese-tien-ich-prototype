...
(function(){
  ...
  const normalize=(raw,p)=>({scope:'all',journey:'all',roundtripCheck:'outbound',airlineMode:'all',airlines:[],routeMode:'all',routes:[],bookingStart:p.start,bookingEnd:p.end,flightDateMode:'all',flightStart:'',flightEnd:'',excludedDates:[],directDiscount:true,discountType:'amount',discount:0,minimum:0,maximum:null,...raw});
  const validate=(c,p)=>{
  ...
  window.flightConditionForm=()=>{
    ...
    modal('Điều kiện chương trình tiếp thị vé máy bay',`<form id="flightConditionForm">
      ...
      <section><h3>1. Phạm vi chuyến bay</h3><div class="fields">
      ${select('Phạm vi','scope',Object.entries(scopes),c.scope)}
      ...
      <div id="flightDiscountFields"><div class="fields">${select('Hình thức giảm','discountType',[['amount','Giảm số tiền'],['percent','Giảm theo %']],c.discountType)}${field('Mức giảm','discount','number',c.discount||'','min="1" step="1"')}</div>
      ${field('Giá trị đơn tối thiểu (đ, tùy chọn)','minimum','number',c.minimum||'','min="0" step="1"')}
      <label class="field" id="flightMaximum">Giảm tối đa mỗi đơn (đ)<input name="maximum" type="number" min="1" step="1" value="${e(c.maximum||'')}"></label>
      <p class="muted">Giảm một lần trên mỗi đơn đủ điều kiện, không nhân đôi cho vé khứ hồi. Mức % tính trên tiền vé đủ điều kiện, không gồm thuế, phí hoặc dịch vụ mua thêm.</p></div></section>
    ...
    const toggle=()=>{
      ...
      const discount=f.directDiscount.checked,percent=f.discountType.value==='percent';
    ...
    for(const name of ['journey','airlineMode','routeMode','flightDateMode','directDiscount','discountType'])f.elements[name].onchange=toggle;
    drawRoutes();drawDates();toggle();
    f.onsubmit=event=>{
      event.preventDefault();
      const fd=new FormData(f),direct=f.directDiscount.checked;
      const next={scope:f.scope.value,journey:f.journey.value,roundtripCheck:f.journey.value==='oneway'?'outbound':f.roundtripCheck.value,airlineMode:f.airlineMode.value,airlines:f.airlineMode.value==='selected'?fd.getAll('airlines'):[],routeMode:f.routeMode.value,routes:f.routeMode.value==='selected'?routes:[],bookingStart:f.bookingStart.value,bookingEnd:f.bookingEnd.value,flightDateMode:f.flightDateMode.value,flightStart:f.flightDateMode.value==='range'?f.flightStart.value:'',flightEnd:f.flightDateMode.value==='range'?f.flightEnd.value:'',excludedDates:[...excluded].sort(),directDiscount:direct,discountType:direct?f.discountType.value:'amount',discount:direct?Number(f.discount.value):0,minimum:direct?Number(f.minimum.value||0):0,maximum:direct&&f.discountType.value==='percent'?Number(f.maximum.value):null};
