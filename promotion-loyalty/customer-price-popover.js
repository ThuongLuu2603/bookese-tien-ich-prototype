...
function priceBreakdown(q,qty=1,atCheckout=false){
 ...
 const original=q.base*qty,discount=Math.max(0,original-total),discountPercent=original?Math.round(discount/original*1000)/10:0;
 const percentLabel=Number.isInteger(discountPercent)?discountPercent.toLocaleString('vi-VN'):discountPercent.toLocaleString('vi-VN',{maximumFractionDigits:1});
 ...
 return `<div class="pp-row"><span>Giá gốc</span><del>${money(original)}</del></div>${discount?`<div class="pp-applied"><div class="pp-row"><span>Ưu đãi phòng · Giảm ${percentLabel}%</span><span>−${money(discount)}</span></div><small>Tính chung cho ${nights()} đêm · trước mã và xu</small></div>`:''}<div class="pp-row pp-section"><span>Giá phòng</span><span>${money(net)}</span></div><div class="pp-row pp-section"><span>Thuế & Phí</span><span>${money(service+vat)}</span></div><div class="pp-sub"><div class="pp-row"><span>Phí dịch vụ 5%</span><span>${money(service)}</span></div><div class="pp-row"><span>Thuế VAT 10%</span><span>${money(vat)}</span></div></div><div class="pp-total"><div class="pp-row"><span>Tổng tiền phòng</span><span>${money(total)}</span></div><em>(Bao gồm Thuế & phí cho ${nights()} đêm, ${qty} phòng)</em></div>${atCheckout?`<div class="pp-checkout-adjustments"><div class="pp-row"><span>Mã Bookese${voucher?' · '+esc(customer.code):''}</span><span>−${money(voucher)}</span></div><div class="pp-row"><span>Xu sử dụng</span><span>−${money(used)}</span></div><div class="pp-row pp-payable"><b>Tổng thanh toán</b><b>${money(payable)}</b></div></div>`:''}<div class="pp-row pp-coins"><div>Xu nhận sau lưu trú<br><em>${customer.tier<0?'Đăng nhập để nhận xu':'Hạng “'+['Thành viên','Thân thiết','Trung thành'][customer.tier]+'”'}</em></div><span>${customer.tier<0?'—':'+'+coins.toLocaleString('vi-VN')+' xu'}</span></div>`;
}
